  ...
  return (
    <div className={`min-h-[calc(100vh-64px)]`}>
      <Parallax
        ...
        className={`min-h-[calc(100vh-64px)] flex flex-col justify-center ${index > 0 ? "pt-[64px]" : ""
          }`}
      >
        <>
          ...
          <div className="flex gap-2 mt-6 flex-wrap">
            <p className="text-[12px] md:text-base text-white">
              {data.links.heading === "deploy" ? <button className="px-4 py-2 bg-gray-900 rounded-lg"><a href={data.links.url} target={"_blank"} rel="noreferrer" className="flex gap-3 items-center">Preview <BiLinkExternal /></a></button> : data.links.heading === "github" ? <a href={data.links.url} target={"_blank"} rel="noreferrer" className="flex gap-3 items-center px-4 py-2 bg-gray-900 rounded-lg">View Code <BsGithub /></a> : null}
            </p>
          </div>
